...
export default async function AdminDashboard() {
  ...
  const { data: clients } = await supabase
    ...
    .select('user_id, preferred_name, role, pipeline_status, monthly_income, liquid_savings, retirement_age, cpf_oa, cpf_sa, cpf_ma')
    .eq('role', 'client')
    .order('created_at', { ascending: false })
    .returns<ClientRow[]>()

  const total = clients?.length ?? 0
  const active = clients?.filter((c) => c.pipeline_status === 'active').length ?? 0
  const withIncome = clients?.filter((c) => Number(c.monthly_income) > 0).length ?? 0
  const avgIncome = withIncome > 0
    ? (clients?.reduce((s, c) => s + Number(c.monthly_income), 0) ?? 0) / withIncome
    : 0
    ...
    <div style={{ padding: 'clamp(20px, 4vw, 40px) clamp(16px, 4vw, 48px)', fontFamily: "'Cabinet Grotesk', sans-serif", maxWidth: 1200 }}>
      ...
      <div className="grid-4col" style={{ gap: 14, marginBottom: 36 }}>
        {[
          ...
          { label: 'Profiles Complete', value: String(withIncome), sub: `${total - withIncome} pending` },
          ...
          <div key={label} style={{
            ...
            <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#c4a882', margin: '0 0 8px' }}>
              {label}
            </p>
            <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, fontWeight: 700, color: '#2a1f1a', margin: '0 0 4px' }}>
              {value}
            </p>
